...
class R1_mAP_unified():
    def __init__(self, num_query, max_rank=50, feat_norm='yes'):
        super(R1_mAP_unified, self).__init__()
        self.num_query = num_query
        self.max_rank = max_rank
        self.feat_norm = feat_norm
    ...
    def update(self, output):
        feat_RGB,feat_NI,feat_TI,feat, pid, camid, sceneid, img_path = output
        self.feats_RGB.append(feat_RGB)
        self.feats_NI.append(feat_NI)
        self.feats_TI.append(feat_TI)
        self.feats.append(feat)
        self.pids.extend(np.asarray(pid))
        self.camids.extend(np.asarray(camid))
        self.sceneids.extend(np.asarray(sceneid))
    ...
    def compute(self):
        feats_RGB = torch.cat(self.feats_RGB, dim=0)
        feats_NI = torch.cat(self.feats_NI, dim=0)
        feats_TI = torch.cat(self.feats_TI, dim=0)
        feats = torch.cat(self.feats, dim=0)
        if self.feat_norm == 'yes':
            # print("The test feature is normalized")
            feats_RGB = torch.nn.functional.normalize(feats_RGB, dim=1, p=2)  # along channel
            feats_NI = torch.nn.functional.normalize(feats_NI, dim=1, p=2)  # along channel
            feats_TI = torch.nn.functional.normalize(feats_TI, dim=1, p=2)
            feats = torch.nn.functional.normalize(feats, dim=1, p=2)
        #######RGB-->NI 
        # query
        qf_RGB = feats_RGB[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])

        q_sceneids = np.asarray(self.sceneids[:self.num_query])  # zxp
        # gallery
        gf_NI = feats_NI[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
        g_camids = np.asarray(self.camids[self.num_query:])

        g_sceneids = np.asarray(self.sceneids[self.num_query:])  # zxp

        m, n = qf_RGB.shape[0], gf_NI.shape[0]
        distmat = torch.pow(qf_RGB, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_NI, 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_RGB, gf_NI.t())
        distmat = distmat.cpu().numpy()
        cmc_R2N, mAP_R2N = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #######RGB-->TI 
        # query
        qf_RGB = feats_RGB[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        q_sceneids = np.asarray(self.sceneids[:self.num_query])  # zxp
        # gallery
        gf_TI  = feats_TI[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
        g_camids = np.asarray(self.camids[self.num_query:])

        g_sceneids = np.asarray(self.sceneids[self.num_query:])  # zxp

        m, n = qf_RGB.shape[0], gf_TI.shape[0]
        distmat = torch.pow(qf_RGB, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_TI , 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_RGB, gf_TI.t())
        distmat = distmat.cpu().numpy()
        cmc_R2T, mAP_R2T = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #######NI-->RGB
        # query
        qf_NI = feats_NI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])

        q_sceneids = np.asarray(self.sceneids[:self.num_query])  # zxp
        # gallery
        gf_RGB  = feats_RGB[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
        g_camids = np.asarray(self.camids[self.num_query:])

        g_sceneids = np.asarray(self.sceneids[self.num_query:])  # zxp

        m, n = qf_NI.shape[0], gf_RGB.shape[0]
        distmat = torch.pow(qf_NI, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_RGB , 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_NI, gf_RGB.t())
        distmat = distmat.cpu().numpy()
        cmc_N2R, mAP_N2R = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #######NI-->TI
        # query
        qf_NI = feats_NI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        m, n = qf_NI.shape[0], gf_TI.shape[0]
        distmat = torch.pow(qf_NI, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_TI, 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_NI, gf_TI.t())
        distmat = distmat.cpu().numpy()
        cmc_N2T, mAP_N2T = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #######TI-->RGB
        # query
        qf_TI = feats_TI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])
        ...
        m, n = qf_TI.shape[0], gf_RGB.shape[0]
        distmat = torch.pow(qf_TI, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_RGB , 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_TI, gf_RGB.t())
        distmat = distmat.cpu().numpy()
        cmc_T2R, mAP_T2R = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #######TI-->NI
        # query
        qf_TI = feats_TI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        m, n = qf_TI.shape[0], gf_NI.shape[0]
        distmat = torch.pow(qf_TI, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                  torch.pow(gf_NI, 2).sum(dim=1, keepdim=True).expand(n, m).t()
        distmat.addmm_(1, -2, qf_TI, gf_NI.t())
        distmat = distmat.cpu().numpy()
        cmc_T2N, mAP_T2N = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        #RGB+NI+TI------>RGB+NI+TI
        # query
        qf = feats[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        distmat.addmm_(1, -2, qf, gf.t())
        distmat = distmat.cpu().numpy()
        cmc_fused, mAP_fused = eval_func_msrv(distmat, q_pids, g_pids, q_camids, g_camids, q_sceneids, g_sceneids)
        return cmc_R2N,mAP_R2N,cmc_R2T, mAP_R2T, cmc_N2R, mAP_N2R, cmc_N2T, mAP_N2T, cmc_T2R, mAP_T2R, cmc_T2N, mAP_T2N, cmc_fused, mAP_fused
...
class R1_mAP_eval_unified():
    def __init__(self, num_query, max_rank=50, feat_norm=True, reranking=False):
        super(R1_mAP_eval_unified, self).__init__()
        self.num_query = num_query
        self.max_rank = max_rank
        self.feat_norm = feat_norm
        self.reranking = reranking
    ...
    def update(self, output):  # called once for each batch
        feat_RGB,feat_NI,feat_TI,feat, pid, camid, img_paths = output
        self.feats_RGB.append(feat_RGB.cpu())
        self.feats_NI.append(feat_NI.cpu())
        self.feats_TI.append(feat_TI.cpu())
        self.feats.append(feat.cpu())
        self.pids.extend(np.asarray(pid))
        self.camids.extend(np.asarray(camid))
        # img_paths should be a list of image names, not full paths
    ...
    def compute(self):  # called after each epoch
        ...
        if self.feat_norm:
            ...
            feats_RGB = torch.nn.functional.normalize(feats_RGB, dim=1, p=2)  # along channel
            feats_NI = torch.nn.functional.normalize(feats_NI, dim=1, p=2)  # along channel
            feats_TI = torch.nn.functional.normalize(feats_TI, dim=1, p=2)  # along channel
            feats = torch.nn.functional.normalize(feats, dim=1, p=2)  # along channel
        #######RGB-->NI 
        # query
        qf_RGB = feats_RGB[:self.num_query]
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_RGB , gf_NI, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_RGB, gf_NI)

        cmc_R2N, mAP_R2N = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #######RGB-->TI 
        # query
        qf_RGB = feats_RGB[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_RGB, gf_TI, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_RGB, gf_TI)

        cmc_R2T, mAP_R2T = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #######NI-->RGB 
        # query
        qf_NI = feats_NI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_NI, gf_RGB, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_NI, gf_RGB)

        cmc_N2R, mAP_N2R = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #######NI-->TI 
        # query
        qf_NI = feats_NI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_NI, gf_TI, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_NI, gf_TI)

        cmc_N2T, mAP_N2T = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #######TI-->RGB
        # query
        qf_TI = feats_TI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_TI, gf_RGB, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_TI, gf_RGB)

        cmc_T2R, mAP_T2R = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #######TI-->NI
        # query
        qf_TI = feats_TI[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        ...
        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_TI, gf_NI, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_TI, gf_NI)

        cmc_T2N, mAP_T2N = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
        #RGB+NI+TI------>RGB+NI+TI
        # query
        qf_fused = feats[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])

        # gallery
        gf_fused = feats[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
        g_camids = np.asarray(self.camids[self.num_query:])

        if self.reranking:
            print('=> Enter reranking')
            distmat = re_ranking(qf_fused, gf_fused, k1=50, k2=15, lambda_value=0.3)
        else:
            # print('=> Computing DistMat with euclidean_distance')
            distmat = euclidean_distance(qf_fused, gf_fused)

        cmc_fused, mAP_fused = eval_func(distmat, q_pids, g_pids, q_camids, g_camids)
