...
class ImageDataset_CA(Dataset):
    def __init__(self, dataset, transform_rgb=None,transform_rgb1=None,transform_thermal=None):
        self.dataset = dataset
        self.transform_rgb = transform_rgb
        self.transform_rgb1 = transform_rgb1
    ...
    def __getitem__(self, index):
        ...
        # if self.transform is not None:
        #     img = [self.transform(img) for img in img3]
        img = [self.transform_rgb(img3[0]),self.transform_rgb1(img3[0]),self.transform_thermal(img3[1]),self.transform_thermal(img3[2])]
        if type(img_path) == type("This is a str"):
            return img, pid, camid, trackid, img_path.split('/')[-1]
        else:
            return img, pid, camid, trackid, img_path[0].split('/')[-1]
